...
import { servicesMap, statusMap } from "@/data/Maps";
import classes from "./BookingTable.module.css";
import Link from "next/link";
import { formatDate } from "@/utils/helper";

export default function BookingTable({ bookings, loading = false }) {
  return (
          ...
          <tbody>
            {bookings.map((booking, index) => (
              <tr key={index}>
                ...
                <td>{booking?.clients[0]?.name}</td>
                <td>{booking?.companyName}</td>
                <td>{servicesMap[booking.services[0]?.type]}</td>
